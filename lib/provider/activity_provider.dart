...
class ActivityProvider extends ChangeNotifier {
  ...
  Future<void> loadSavedActivities() async {
    try {
      ...
      snackBarFailed(content: 'Error loading saved activities: ${e.toString()}');
    } finally {
      isLoading = false;
      notifyListeners();
    }
  ...
  Future<void> loadSavedActivitiesByTypes(String type) async {
    try {
      ...
      final savedActivities = _localDbService.getHistory();
      if (savedActivities != null && savedActivities.isNotEmpty) {
        _list = savedActivities.where((element) => element.type!.toLowerCase() == type.toLowerCase()).toList();
      }
    } catch (e) {
      snackBarFailed(content: 'Error loading saved activities: ${e.toString()}');
    } finally {
  ...
  Future<void> fetchActivityByType(String type) async {
    try {
      ...
      activityModel = await ActivityRepository.instance.getActivityModelByType(type.toLowerCase());
