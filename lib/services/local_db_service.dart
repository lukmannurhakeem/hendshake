...
import 'package:hendshake/constants/local_db_constant.dart';
...
class LocalDBService {
  ...
  String getRefreshToken() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_REFRESH_TOKEN, defaultValue: '');
  }

  String getUsername() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_USERNAME, defaultValue: '');
  }

  String getEmail() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_EMAIL, defaultValue: '');
  }

  String getUid() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_UID, defaultValue: '5ad701ef4cedfd0001022276');
  }

  String getLocale() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_LANGUAGE, defaultValue: 'en');
  }

  bool isLoggedIn() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.KEY_IS_LOGGED_IN, defaultValue: false);
  }
  ...
  Future<void> setRefreshToken(String token) async {
    await Hive.box(ConstantValue.dbName).put(DBConstant.KEY_REFRESH_TOKEN, token);
  }
  ...
  bool getIs24Hours() {
    return Hive.box(ConstantValue.dbName).get(DBConstant.IS_24_HOURS, defaultValue: false);
  }
  ...
  Future<void> setHistory(List<ActivityModel>? activities) async {
    ...
    final List<String> jsonStringList = activities.map((activity) => json.encode(activity.toJson())).toList();

    // Store the JSON string list
    await Hive.box(ConstantValue.dbName).put(DBConstant.KEY_LIST, jsonStringList);
  }

  // Updated method to retrieve list of ActivityModel objects
  List<ActivityModel>? getHistory() {
    final List<dynamic>? jsonStringList = Hive.box(ConstantValue.dbName).get(DBConstant.KEY_LIST);
  ...
  Future<void> setSelectedTypes(String types) async {
    if (types.isEmpty) {
      await Hive.box(ConstantValue.dbName).delete(DBConstant.KEY_SELECTED_TYPES);
      return;
    }

    await Hive.box(ConstantValue.dbName).put(DBConstant.KEY_SELECTED_TYPES, types);
  }

  String? getSelectedTypes() {
    print('Henlo 3');

    final String? storedTypes = Hive.box(ConstantValue.dbName).get(DBConstant.KEY_SELECTED_TYPES);
